...
| **DuckDB** | — | 組込み OLAP データベース。Parquet 統合・SQL 結合に使用。Serverless First（常時 DB サーバー不要）。 |
...
| **PoC** | Proof of Concept | 概念実証。Leaflet 地図 + 重みスライダーで \(J_i\) を体験する静的ビューア。 |
| **Open Data** | オープンデータ | 再利用可能な公開データ。Open-Meteo、OpenStreetMap、行政 OD を使用。 |
